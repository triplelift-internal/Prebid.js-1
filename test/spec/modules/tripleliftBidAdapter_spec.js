  ...
  describe('buildRequests', function () {
    ...
    this.beforeEach(() => {
      bidRequests = [
        ...
        // instream video; valid placement
        {
          bidder: 'triplelift',
          params: {
            inventoryCode: 'outstream_test',
    ...
    it('should check for valid instream placement values', function () {
      const request = tripleliftAdapterSpec.buildRequests(bidRequests, bidderRequest);
      const payload = request.data;
      // instream video; valid placement
      expect(payload.imp[7]).to.not.have.property('banner');
      expect(payload.imp[7]).to.have.property('video');
      expect(payload.imp[7].video).to.exist.and.to.be.a('object');
      expect(payload.imp[7].video.placement).to.equal(1);
      // instream video; valid placement
      expect(payload.imp[8]).to.not.have.property('banner');
      expect(payload.imp[8]).to.have.property('video');
      expect(payload.imp[8].video).to.exist.and.to.be.a('object');
      expect(payload.imp[8].video.placement).to.equal(5);
      // instream video; invalid placement
      ...
      expect(payload.imp[10].video.placement).to.equal(1);
    });
